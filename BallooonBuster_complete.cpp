...
int main()
{
    ...
    if (!popBuffer.loadFromFile("balloonpop.ogg")) { //try to load the sound data into popbuffer
        cout << "coul not load balloonpop.ogg" << endl; // if failed, tell us
    ...
    if (!music.openFromFile("circus.ogg")) { //open the file to stream the music data from
        cout << "Failed to load circus.ogg "; //tell us if the open failed
    ...
    LoadTex(redTex, "images/red_balloon.png"); // load the red balloon image data into the Texture
